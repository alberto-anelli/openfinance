...
export default function createTransactionRouter(repo) {
    const router = Router();
    ...
    router.get('/transactions', async (req, res) => {
        try {
            ...
            const transactions = await repo.list(filter);
    ...
    router.patch('/transactions/:id', async (req, res) => {
        try {
            const { cleaned, errors } = validatePatch(req.body);
            if (errors.length > 0) {
                res.status(400).json({
                    error: { code: 'VALIDATION_ERROR', message: errors.map(e => e.message).join('; ') },
                });
            ...
            const tx = await repo.update(req.params.id, cleaned);
            res.json(tx);
        }
        catch (err) {
